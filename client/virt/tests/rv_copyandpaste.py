"""
rv_copyandpaste.py - Testing copying from clipboard from client to guest
                     or vice versa
...
def place_img_in_clipboard(session_to_copy_from, interpreter, script_call,
    ...
    try:
        ...
        logging.info("------------ End of script output of the Copying"
                     " Session ------------")
...
def run_rv_copyandpaste(test, params, env):
    """
    Testing copying and pasting to from a client to guest
    ...
    if (cp_disabled_test == "yes"):
        #This is a negative test, only need to check if it is being copied
        #guest to client or client to guest
        if "client_to_guest" in test_type:
            if "image" in test_type:
    ...
    #The following if statement will determine what test to run
    #(Negative or Positive, Guest to Client or Client to Guest Copying)
